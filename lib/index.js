...
module.exports = function(inputPath, outputFile) {
  ...
  var errorStream = highland(presetNames)
  ...
  .filter(function(error) {
    var rule = error.rule;
    return rule.indexOf('validateIndentation') > -1;
  })
  .flatMap(function(error) {
    ...
    .reduce(function(a, b) {
      ...
      console.log('b');
      console.log(b);
      return highland.flatMap(function() {
  ...
  /*
  var requireMultipleVarDeclStream = errorStream.fork()
  .filter(function(error) {
    var rule = error.rule;
    return rule.indexOf('requireMultipleVarDecl') > -1;
  })
  .map(function(error) {
    //customJscsrc[rule] = true;
    //customJscsrc[rule] = 'onevar';
    delete customJscsrc.requireMultipleVarDecl;
  });
  //*/
  ...
  requireDisallowStream
    .concat(requireMultipleVarDeclStream)
    .pipe(fs.createWriteStream(outputFile));
    //.pipe(process.stdout);
  //*/

  /*
  validateQuoteMarksStream.concat(spacesInsideArrayBracketsStream)
    .concat(requireMultipleVarDeclStream)
    .concat(requireDisallowStream)
    .pipe(fs.createWriteStream(outputFile));
    //.pipe(process.stdout);
  //*/
  ...
  //errorStream.fork().pipe(process.stdout);
    ...
    return compareOptionsSingleVariable(existingOption, alternateOption)
      .map(function(bestOptionType) {
        ...
        if (bestOptionType === 'alternate') {
          ...
          customJscsrc[alternateRuleName] = alternateRuleValue;
        }
        return customJscsrc;
      });
  }
